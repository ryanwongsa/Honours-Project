...
/*
	Initialise the neuron input layers
*/
...
/*
	Initialise the neuron hidden layers
*/
...
/*
	Initialise the neuron output layers
*/
...
void NeuronLayers :: initialiseLinks()
{
	...
	for(int i=0;i<numInput;i++)
	{
		...
		for(int j=0;j<numHiddenNodes;j++)
		{
			links[0][i].push_back(Links (distribution(generator)));//float (-1/(float)3 + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(2/(float)3))))));
		}
	}
	...
	for(int a=1;a<numHiddenLayers;a++)
	{
		for(int i=0;i<numHiddenNodes;i++)
		{
			...
			for(int j=0; j<numHiddenNodes;j++)
			{
				links[a][i].push_back(Links (distribution(generator)));//float (-1/(float)3 + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(2/(float)3))))));
			}
		}
	}
	...
	for(int i=0;i<numHiddenNodes;i++)
	{
		...
		{
			links[numHiddenLayers][i].push_back(Links (distribution(generator)));//float (-1/(float)3 + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(2/(float)3))))));
		}
	}
}
...
/*
	Returns the error for the output
*/
float NeuronLayers :: getErrorOutput(int i)
{
	...
	// if( abs(difference)<0.2*abs(neuron[numNeuronLayers-1][i].getTarget()-neuron[numNeuronLayers-1][i].getTargetDeviation() ) )
